...
- Friend discovery: username search only in v1. Each user has a shareable profile URL (`/@[username]`) with a "Send friend request" button and a copy-link affordance. No email-based invite flow.
...
- Auth mechanism: JWT-based sessions via Auth.js v5. The Credentials provider requires the JWT strategy (database sessions are not supported for credentials auth). Session state is encoded in a signed, HttpOnly cookie and verified without a DB round-trip. Logout clears the cookie.
